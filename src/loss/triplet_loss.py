# Source: https://github.com/alfonmedela/triplet-loss-pytorch
...
def pairwise_distance_torch(embeddings, device):
    ...
    pairwise_distances_squared = torch.max(pairwise_distances_squared,
    ...
    mask_offdiagonals = torch.ones(
        (pairwise_distances.shape[0],
         pairwise_distances.shape[1])) - torch.diag(
             torch.ones(pairwise_distances.shape[0]))
    pairwise_distances = torch.mul(pairwise_distances.to(device),
...
def TripletSemiHardLoss(y_true, y_pred, device, margin=1.0):
    ...
    masked_minimums = torch.min(torch.mul(pdist_matrix_tile - axis_maximums[0],
                                          mask),
                                dim=1,
    ...
    masked_maximums = torch.max(torch.mul(pdist_matrix - axis_minimums[0],
                                          adjacency_not),
                                dim=1,
                                keepdim=True)[0] + axis_minimums[0]
